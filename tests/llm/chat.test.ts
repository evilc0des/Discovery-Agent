...
const { mockStreamObject, mockGenerateText, mockChat } = vi.hoisted(() => ({
  mockStreamObject: vi.fn(),
  mockGenerateText: vi.fn(),
  mockChat: vi.fn((model: string) => `do:chat:${model}`),
}));
...
vi.mock('@/lib/llm/provider', () => ({
  digitalocean: { chat: mockChat },
  DO_MODEL: 'deepseek-v4-pro',
}));
...
describe('generateFallbackResponse', () => {
  ...
  it('uses generateText for text-only response', async () => {
    ...
    expect(callArgs.model).toBe('do:chat:deepseek-v4-pro');
